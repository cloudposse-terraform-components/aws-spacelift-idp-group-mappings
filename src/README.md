...
| <a name="input_idp_group_mappings"></a> [idp\_group\_mappings](#input\_idp\_group\_mappings) | Map of IDP group mappings with role names and associated spaces. The key is the IDP group name. | <pre>map(object({<br/>    spacelift_role_name = string<br/>    spaces              = list(string)<br/>  }))</pre> | `{}` | no |
